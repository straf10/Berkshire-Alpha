...
- **Greeks and IV come from Alpaca.** The option snapshot/chain endpoint returns greeks and implied volatility per contract — we do **not** implement Black-Scholes. That saves half a day; don't re-derive it. Aggregate portfolio greeks are summed from these per-leg values.
- **Data tier:** the free plan's options feed is delayed (indicative), not real-time OPRA. Budget for it — wider limit tolerances, and never assume the quote you screened on is the quote you'll fill at. Confirm the actual feed our account gets on Day 1 and state it honestly in the one-pager.
