...
## Addendum, 2026-09-02 evening — LLY exit rehearsal, and two new findings

Out of scope of the `3ec65f9~1..c224c29` diff above — this is new material from running `scripts/rehearse_exit.py` against the live judged account tonight, ahead of tomorrow's forced closes (`TIME_STOP_2DTE` at the first tick, `UNWIND` at 15:30 ET).

### Rehearsal result: LLY (trade 8), live quotes as of 2026-09-02 ~18:45 UTC

```
entry_net_mid=6.69  width=5.0
  LLY260904P01160000 (SELL 1160P): bid=8.44 ask=13.66 mid=11.05  spread=47.2%  <== > 25% OF MID
  LLY260904P01165000 (BUY  1165P): bid=10.00 ask=17.08 mid=13.54  spread=52.3%  <== > 25% OF MID
closing plan: net_mid=-2.49  net_natural=3.66  cap=1.82
  fill=mid      -2.49   realized_pnl = -$1,680.00
  fill=natural   3.66   realized_pnl = -$4,140.00
  fill=cap       1.82   realized_pnl = -$3,404.00   <-- the number to plan around
```

Both legs are independently over `MAX_QUOTE_SPREAD_PCT` right now — the same illiquidity signature as the original trade-8 entry, on the exit side, the night before it's forced closed twice. The realized loss is between **-$1,680 and -$4,140** under every modeled fill, worst case at the cap. `entry_net_mid` here is `final_limit` (6.69), not `fill_price` (6.65) — `_open_trades` ([main.py:232](agent/main.py#L232)) reads the last submitted limit, not the actual average fill; a $0.04/share, $16-total gap on this trade, small here but worth noting as the same category of "modeled, not realized, number feeding a downstream decision" as P1-1 below.

NVDA (trade 4) was also rehearsed (still shows open in the DB), but has in fact already closed at the broker — see the Bug A reconciliation work; `scripts/reconcile_closes.py` will correct this before it matters again. Its rehearsed `natural` fill (-2.05, +$224.00) matches the broker's real fill exactly, which is a useful internal-consistency check on the rehearsal script itself.

### Finding — `reduce_only` doesn't reduce

**Mechanism.** `reduce_only` is read in exactly one place: [gates.py:137](agent/risk/gates.py#L137), `if ctx.reduce_only: return _reject(GateReason.REDUCE_ONLY)` — inside `evaluate()`, the **entry** gate. It stops new positions from being sized and opened. `evaluate_exit()` ([exits.py:28-31](agent/risk/exits.py#L28-L31)) — the function that decides whether an *existing* position closes — takes `is_credit, entry_net_mid, current_net_mid, max_profit_per_spread, dte, unwind_triggered`. No greeks parameter exists in that signature at all, and no caller threads portfolio delta into `exit_tick` ([main.py:591-654](agent/main.py#L591-L654)) either. So the one condition `reduce_only` is meant to signal — the portfolio is over its delta limit — has no path into the function that decides what closes and when. Entries pause; the thing that was actually breached does not move any faster toward compliance than it would have anyway (time stops, profit targets, and stop losses fire on their own schedule, unrelated to why `reduce_only` turned on).

**Live confirmation, right now:** `GET /greeks/latest` — `delta_dollars=-37,245.84`, `delta_limit=14,342.13` (`PORTFOLIO_DELTA_PCT × equity` = `0.15 × ~$95,614`, [config.py:134](agent/config.py#L134)), `breached=true`. `reduce_only` is active on this account at this moment, entirely on the strength of the LLY position, and nothing in the exit path is aware of it.

### Finding — notional-vs-max-loss mismatch

Sizing (`MAX_RISK_PER_TRADE`/`MAX_AGGREGATE_RISK`, [gates.py:188-195](agent/risk/gates.py#L188-L195)) is denominated in `max_loss_per_spread` — a defined-risk quantity bounded by the spread's own strike width. `PORTFOLIO_DELTA_LIMIT` is denominated in delta-weighted dollar notional: `delta_dollars = Σ delta_leg × qty_leg × spot × 100` ([greeks.py:94](agent/risk/greeks.py#L94)), compared against `PORTFOLIO_DELTA_PCT × equity`. These are not the same unit, and nothing reconciles them.

LLY is the live example. Fill-derived max loss (`_max_loss_from_fill`, [main.py:171-187](agent/main.py#L171-L187): a debit structure's max loss is `fill_price × 100`) is `6.65 × 100 = $665.00`/spread × `filled_qty 4` = **$2,660.00** total, bounded and known at entry. The short leg alone (1160 strike, qty 4, spot ≈1161) carries `4 × 100 × 1161 ≈` **$464,400** of notional — on a $95.6k-equity account. A $2,660 worst-case-defined-risk position is carrying roughly 4.9× the account's own equity in notional. That is not a bug in either limit; `MAX_RISK_PER_TRADE` and `PORTFOLIO_DELTA_LIMIT` are each individually correct and doing what they're specified to do. It's an unmodelled interaction between two correct limits, on a high-notional, near-the-money, single-underlying name — exactly the shape a validation framework should have surfaced before it showed up live on the judged account.

**Live-DB footnote on the $2,660 figure — deploy-time check, resolved: historical row, not a live regression.** `trades.max_loss_per_spread` for trade 8 currently persists **$194.00** (the pre-fill, net-mid-basis estimate), not the fill-derived $665.00 — the exact case `_max_loss_from_fill`'s own docstring cites as its motivating example. That raises the real question: did the fix (`3ec65f9`, merged `c224c29`) deploy before or after trade 8 entered? Commit time isn't deploy time, so checked the actual deploy: `.github/workflows/ci-cd.yml`'s `deploy-backend` job runs `railway up --service ... --detach` on push to `main`; the real GitHub Actions run for `c224c29` (`gh run view 33566580537`) shows that job triggered at 2026-09-01T22:29:48Z and its `railway up --detach` step completing at **2026-09-01T22:30:53Z**. Trade 8 entered **2026-09-01T17:16:16Z** — over five hours before the deploy step even ran (`--detach` returns once the Railway build is *kicked off*, not once it's live, so actual cutover was if anything later still). The fix was not deployed yet when trade 8 entered: the stale row is exactly what's expected, and nothing in the code currently running is failing to fire. No trade has entered since the deploy to exercise it live (2026-09-02 produced zero ENTER decisions before this rehearsal), but the call site is intact and unchanged at HEAD.

The **consequence** is still live, though, independent of when the bug was fixed: `_open_defined_risk()` ([main.py:190](agent/main.py#L190)) reads whatever `max_loss_per_spread` a row currently holds, trade 8's was never backfilled, and the aggregate-risk ledger is under-counting LLY's contribution by `(665−194) × 4 = $1,884` *tonight*, sizing every hypothetical new candidate against a number that's $1,884 too low. `scripts/reconcile_closes.py`'s remit is realized P&L on already-*closed* trades — LLY is still open, so today's Bug A work doesn't touch this row. If it needs fixing before tomorrow, it's a one-row backfill (`storage_write.update_trade_result`'s `max_loss_per_spread` param, the same write the live path already uses), not a code change.

### Two things found by accident, worth acting on before tomorrow

- **A closing order for LLY has been resting, unfilled, at the broker for 5+ hours.** `alpaca order list --status open` shows order `55df0020-...`, `buy_to_close`/`sell_to_close`, `limit_price=-2.04`, submitted `2026-09-02T13:30:02Z` (5h14m ago as of this check), `filled_qty=0`. This is almost certainly the walk this session's own `exit_tick` submitted and then lost track of (nothing in `trades` records its order id — `exit_tick` only persists on a *successful* close, [main.py:635-647](agent/main.py#L635-L647)) — this is the concrete, "sat unfilled for hours" evidence the day's diagnosis was built on. It is still live right now. Flagging for a human decision, not acting on it myself.
- **The Alpaca CLI's `--symbols` filter does not work on multi-leg orders — the best catch of the night, and a validation-of-tooling finding, not a validation-of-strategy one.** `alpaca order list --status closed --symbols <LLY legs> --nested` returned the *NVDA* closing order — confirmed by re-running with a nonexistent symbol and getting the same single most-recent result every time. `--symbols` evidently filters against a top-level `symbol` field that only single-instrument orders have; an `order_class=mleg` order has none, so the filter appears to silently no-op rather than error or return empty. That shape of failure — confidently wrong, never loud — is the dangerous one: `scripts/reconcile_closes.py` ([cli_bridge.py:112-150](agent/execution/cli_bridge.py#L112-L150)), written earlier tonight for Bug A, trusted the flag and would have mis-attributed every reconciled close the moment there was more than one candidate to disambiguate. It only looked correct against NVDA because there was exactly one closed mleg order to find at the time — a second open trade (LLY) is what exposed it. Fixed: filtering moved client-side, matching each leg's own `symbol` inside `legs[]` rather than trusting the CLI flag; pinned with a regression test (`test_list_orders_for_symbols_filters_client_side`) built from this exact failure, and re-verified live afterward — LLY now correctly returns no closed orders, NVDA still reconciles to $224.00. The lesson generalises: a reconciliation script is only as trustworthy as the read path it's built on, and that path was never exercised against more than one candidate until tonight.
