...
- **✅ Fixed 29 Aug.** The account previously recorded as "Judged Account" (id `b1a0e3d2-...`) had a manual `mleg` round-trip test order placed and filled on it on Day 1 (28 Aug), making it a testing account per the rule above. A new, untouched account (`bc8bc895-ec1e-4b9d-9f69-413432024e5e` / `PA3UM9X4MN5X`) was created 29 Aug with $100,000 and Options Level 3 approved, and is now recorded in `README.md` as the judged account. The old account remains the permanent dev/test account. **Remaining step:** confirm the deploy host's (Railway) live env vars point to the new account's keys, not just the local `.env`.
