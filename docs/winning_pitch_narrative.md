...
| "Isn't the LLM just decoration if the gate overrules it?" | "The gate has overruled a 2-of-3 LLM majority twice on record — `NEGATIVE_EDGE` on NVDA (Kelly f* = −0.0575, p_success 0.416) and `EARNINGS_BLACKOUT` on AVGO. And there's an adversarial test, `test_unanimous_approve_of_oversized_trade_rejected`, that fabricates a *unanimous* approval from all three risk personas on an oversized trade and asserts the broker never sees an order." |
...
| Gate overrode LLM majority | **2×** — `NEGATIVE_EDGE` (Kelly f* −0.0575, p_success 0.416), `EARNINGS_BLACKOUT` | decisions 97/147, 135 |
| Persona vote splits | 9 of 10 votes were 2-1; gate ruled independently of the majority | `memory.md` 2026-09-01 audit |
