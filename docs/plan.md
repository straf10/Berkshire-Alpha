...
/agent          Python backend — the actual trading agent
  ...
  /agents       analysts.py (sentiment/news/quant), researchers.py (bull/bear DoC debate + consensus-threshold early termination),
...
3. **Researcher team — Bull vs Bear adversarial debate, under Disagree-or-Commit**
   ...
   - **Consensus-threshold early termination.** After round 1, the orchestrator checks whether both researchers agree (unanimous COMMIT or unanimous DISAGREE). If so, **the debate terminates at round 1** and proceeds directly to the trader (or to a no-trade, for unanimous DISAGREE). Extended rounds on an already-settled question yield diminishing marginal information and burn credits linearly. Contested candidates get round 2; agreed ones don't.
   - **Hard cap: 2 rounds.** Early termination can end it sooner; nothing can extend it.
...
- **Call budget, counted properly.** Analysts run per candidate: 3 × 4 = **12 calls**. Debate/trader/risk run only on the 2 surviving candidates: 2 debate calls (round 1) + 2 more *only if the researchers split* + 1 trader + 3 risk = **6–8 each**, so **12–16**. That's **≈24–28 calls per scan**, ≈48–56 per session, and **≈300–350 across the competition**, with consensus-threshold early termination typically pulling us toward the low end. Comfortable, and only because of three decisions: the 2×/session cadence, narrowing to 2 names before the debate, and early termination on agreement. Fanning the full debate across 4 candidates at a 15-minute cadence would be roughly 40× this — that, not prompt length, is what burns a credit.
...
- **Center: the Reasoning Feed.** Expandable per-ticker cards showing the full chain — analyst evidence with the raw quant values → the bull/bear debate with each agent's **DoC action and cited evidence** → whether consensus-threshold early termination ended the debate early, and the resulting conviction → trader proposal → risk-persona votes → the deterministic gate's approve/reject *with the specific numeric threshold that decided it* → order lifecycle including every walk step and reject. Judges can read a Bear agent DISAGREE-ing on steep skew and blocking a trade, then watch the gate resize the one that passed. This is what turns a black box into an auditable decision log, and it is our strongest asset for "Presentation & Explainability".
...
3. **Day 3 (Sun 30 Aug) — the agent.** Reddit sentiment; the full LLM pipeline (analysts → DoC debate with consensus-threshold early termination → trader → risk team) behind the deterministic fund-manager gate; Pydantic schemas plus the one-retry policy; half-Kelly sizing and the aggregate-Greek gates unit-tested. Dry-run the whole loop against the closed market to shake out crashes.
